...
class Operations(pyfuse3.Operations):
    ...
    def _authenticate(self):
        ...
        server.starttls()
        server.login("ssi20g22@gmail.com", "#SSIbateLARGO!")
        code = systemRandom.randint(100000,999999)
        authcode = str(code)
        msg = MIMEText("Código de autorização: " + authcode)
        msg['Subject'] = 'Código de acesso ao ficheiro/diretoria!'
        msg['From'] = 'ssi20g22@gmail.com'
        msg['To'] = self._user._contact
        server.sendmail('ssi20g22@gmail.com', [self._user._contact], msg.as_string())
        server.quit()
        signal.alarm(TIMEOUT)
        finished = False
        authorized = False
        tries = 1
        inp = input("Código de acesso : ")
        while(not(finished)):
            if(inp == authcode):
                authorized = True 
                signal.alarm(0) 
                ...
                if (tries<=3):
                    print("Código de acesso errado! Digite novamente")
                    inp = input("Código de acesso : ")
                    tries+=1
                else:
                    authorized = False     
    ...
    def sendScanMail(self,scanResults):
        server = smtplib.SMTP('smtp.gmail.com', 587)
        server.starttls()
        server.login("ssi20g22@gmail.com", "#SSIbateLARGO!")
        results = str(scanResults)
        msg = MIMEText(results)
        msg['Subject'] = 'Resultado de scan ao ficheiro!'
        msg['From'] = 'ssi20g22@gmail.com'
        msg['To'] = self._user._contact
        server.sendmail('ssi20g22@gmail.com', [self._user._contact], msg.as_string())
    ...
    async def open(self, inode, flags, ctx):
        print('\n')
        print("Foi enviado o código de acesso para o seu e-mail!")
        print("Insira-o abaixo para aceder ao ficheiro ou diretoria")
        if(self._authenticate()):
            print("Acesso Autorizado , aguarde pelos resultados do scan ao ficheiro...")
            print('\n')
            print("=====================================================================")
            cd = pyclamd.ClamdAgnostic()
            print('\n')
            print("ClamAV Ativo: " + str(cd.ping()))
            print('\n')
            infected=False
            scanResults = cd.scan_file(self._inode_to_path(inode))
            if (scanResults is None):
                print("Este ficheiro não contém virus!")
            else:
                self.sendScanMail(scanResults)
                infected = True
                print("Foi descoberto um vírus na tentativa de abertura do ficheiro!")
                print("Os detalhes do scan foram enviados para o seu email.")
                os.remove(self._inode_to_path(inode))
            ...
            if inode in self._inode_fd_map:
                if (infected is False):
                    ...
                    fd = self._inode_fd_map[inode]
                    self._fd_inode_map.__delitem__(fd)
                    fd.close()
                    self._inode_fd_map.__delitem__(inode)
                    self._fd_open_count[fd] -= 1
                    Operations.flush(self,fh=fd)
